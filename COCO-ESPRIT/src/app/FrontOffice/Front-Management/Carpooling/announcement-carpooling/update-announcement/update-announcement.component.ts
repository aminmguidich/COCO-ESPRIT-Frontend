...
onAddMarker($event:H.map.Marker) {
  let newArr=this.adressesAsStrings.slice(0,this.adressesAsStrings.length-1)
...
console.log(this.announcement.routeAnnCarpooling.adressesRoute)
  this.announcement.routeAnnCarpooling.adressesRoute.push({
    idAdress:-1, streetName:$event.getData(),  latitude :position.lat,
    longitude:position.lng
  });
console.log(this.announcement.routeAnnCarpooling.adressesRoute)
  }
  onRemoveItem($event: number) {
    let removedAdress:Adress=this.announcement.routeAnnCarpooling.adressesRoute[$event]
    this.removedAddresses.push(removedAdress)
    console.log(this.removedAddresses)
    let newArr=this.adressesAsStrings.slice(0,this.adressesAsStrings.length-1).filter((value,index,array)=>
      index!=$event)
     newArr.push("Esprit")
   this.adressesAsStrings=newArr
   ...
   document.getElementById("map")?.scrollIntoView()
  this.announcement.routeAnnCarpooling.adressesRoute=this.announcement.routeAnnCarpooling.adressesRoute.filter((value,index,array)=>{
    return index!=$event
  })
  }
  ...
  console.log(this.announcement.routeAnnCarpooling,"hhihi")

  let adressToMove=this.announcement.routeAnnCarpooling.adressesRoute[$event.previousIndex]
  if($event.previousIndex<$event.currentIndex){
    for(let i=$event.previousIndex;i<$event.currentIndex;i++){
      this.announcement.routeAnnCarpooling.adressesRoute[i]=this.announcement.routeAnnCarpooling.adressesRoute[i+1]
    }
  }else{
    for(let i=$event.previousIndex;i>$event.currentIndex;i--){
      this.announcement.routeAnnCarpooling.adressesRoute[i]=this.announcement.routeAnnCarpooling.adressesRoute[i-1]
  ...
  this.announcement.routeAnnCarpooling.adressesRoute[$event.currentIndex]=adressToMove
  //console.log(this.announcement.routeAnnCarpooling)
  }
  ...
  ngOnInit() {
   ...
   /* date.setFullYear(data.dateCarpoolingAnnouncement[0])
    ...
    console.log(date)
    console.log(data.dateCarpoolingAnnouncement[0],"0")    
    console.log(( (data.dateCarpoolingAnnouncement[1])),"1")      
    console.log(    data.dateCarpoolingAnnouncement[2]
    ,"2")     */ 
  ...
  async updateAnnouncementCarpooling() {
   ...
   let route=await this.routeService.updateRoute(this.announcement.routeAnnCarpooling).toPromise()
